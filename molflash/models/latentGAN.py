...
class Discriminator(nn.Module):
    def __init__(self, data_shape=(1, 2048)):
...
class Generator(nn.Module):
    def __init__(self, data_shape=(1, 2048), latent_dim=None):
...
def compute_gradient_penalty(real_samples,
                                 fake_samples, discriminator):
        """Calculates the gradient penalty loss for WGAN GP"""
        # Random weight term for interpolation between real and fake samples
        alpha = Tensor(np.random.random((real_samples.size(0), 1))).to(dev)
        with torch.enable_grad():
        # Get random interpolation between real and fake samples
            interpolates = (alpha * real_samples +
                            ((1 - alpha) * fake_samples)).requires_grad_(True)
            d_interpolates = discriminator(interpolates)
            fake = Tensor(real_samples.shape[0], 1).fill_(1.0).to(dev)
