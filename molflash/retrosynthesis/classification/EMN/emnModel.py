...
from molflash.retrosynthesis.classification.EMN.emnData import DataModule   # gcnDataModule, gcnDataSource

from molflash.utils.emnPrepro import smile_to_graph
from molflash.models.emn import EMN
from molflash.retrosynthesis.classification.EMN.emnData import DataModule


class emnTask(flash.Task):
    """
    Args:
...
if __name__=="__main__":
    ...
    dm = DataModule(filePath=filePath,transform=smile_to_graph,batch_size=batch_size,splits=splits)
    dm.prepare_data()
    dm.setup()
    model = emnTask(model = eval(modelName), loss_fn=loss_fn, learning_rate=learning_rate,optimizer=eval(optimizer),metrics=metrics)
